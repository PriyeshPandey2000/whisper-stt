...
export function createTrayIconDesktopService(): SetTrayIconService {
	...
	return {
		setTrayIcon: (recorderState: NoteFluxRecordingState) =>
			tryAsync({
				mapErr: (error) =>
...
export function createTrayIconWebService(): SetTrayIconService {
	return {
		setTrayIcon: async (icon: NoteFluxRecordingState) => {
...
async function getIconPath(recorderState: NoteFluxRecordingState) {
	const iconPaths = {
